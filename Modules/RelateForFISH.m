function handles = RelateForFISH(handles)

% Help for the RelateForFish module:
% Category: Object Processing
%
% SHORT DESCRIPTION:
% Assigns relationships: All objects (e.g. speckles) within a parent object
% (e.g. nucleus) become its children. THIS VERSION HAS A BUGFIXED (BY
% BEREND SNIJDER), WHICH IS REQUIRED FOR PROPER PROCESSING OF SPOTS WITH
% THE MODULES OF BATTICH ET AL. 2013
% *************************************************************************
%
...
%BS, POSSIBLE BUGFIX: REMOVE OTHER MEASUREMENT FIELDNAMES...
% NOTE, WE MIGHT WANT TO DELETE THIS AGAIN... IT'S AN INTERESTING FEATURE
MeasurementFieldnames(~cellfun(@isempty,strfind(MeasurementFieldnames,'Texture')) | ...
                        ~cellfun(@isempty,strfind(MeasurementFieldnames,'AreaShape')) | ...
                        ~cellfun(@isempty,strfind(MeasurementFieldnames,'Intensity')) ) = [];
                ...
                for i=1:length(MeasurementFeatures)
                    for j=1:max(max(ParentObjectLabelMatrix))
                        ...
                        if isempty(index)
                            ...
                            index = index(1);
                            if index <= size(Measurements,1)% BS-DEBUG/HACK AVOID WEIRD ERRORS
                                handles.Measurements.(NewObjectName).(FieldName(1:end-8)){handles.Current.SetBeingAnalyzed}(j,i)=mean(Measurements(index,i));
                            else
                                %fprintf('\t   %s: BS-DEBUG: erroneous situation encountenered... \n',mfilename)
                                handles.Measurements.(NewObjectName).(FieldName(1:end-8)){handles.Current.SetBeingAnalyzed}(j,i)=NaN;
                            end
                        end
                    end
                end
